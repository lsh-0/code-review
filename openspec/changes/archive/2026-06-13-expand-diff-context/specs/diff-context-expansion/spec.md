...
A file whose diff carries the `Binary files ... differ` marker SHALL be flagged as binary during diff parsing. A binary file SHALL still appear in the file navigation, but selecting it SHALL show a plain placeholder (for example "binary file") instead of a diff body, and SHALL NOT render hunks, expansion affordances, or fetch the file's blob content.
...
- **THEN** a plain placeholder is shown in place of a diff body, with no hunks and no expansion affordances, and no blob content is fetched for it
