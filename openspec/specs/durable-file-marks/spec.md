# durable-file-marks Specification

## Purpose
Keep the per-file "done" mark honest across sessions: a marked file is one the reviewer has finished
with, so a mark must drop the moment a commit changes that file — even if the change happened while the
app was closed. Each mark records the git blob SHA of the file's committed content at mark-time; on
opening a review, a differing or missing SHA evicts the mark. The trigger is committed changes only;
uncommitted working-tree edits are surfaced separately and do not affect marks.
