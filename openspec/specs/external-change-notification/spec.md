# external-change-notification Specification

## Purpose
Let the reviewer know when another process — an AI agent or the review CLI — has changed the review
state file while the GUI is open, without the view changing underneath them. The tool is built around an
async round-trip (reviewer comments, hands off to an agent, returns later), so a silently stale view is
the failure to avoid; the reviewer is told via a dismissable banner and chooses when to refresh. The
GUI's own writes are distinguished from external ones so they never raise a false notification.
