# comment-reanchoring Specification

## Purpose
Keep a comment tethered to the code it was made against as the diff is recomputed and the underlying
content changes. Each comment carries an ordered history of blob-keyed anchors rather than a bare line
number: its current placement and outdated state are derived from its most-recent anchor. On
reconciliation the system reuses an existing anchor for a known blob SHA before falling back to
exact-then-fuzzy matching of the last good context against the recomputed diff, capturing a fresh
context window on every successful re-anchor. Comments that can no longer be placed render untethered
with a warning, legacy comments upgrade to anchors on load, and deleting an outdated comment removes its
captured context.
