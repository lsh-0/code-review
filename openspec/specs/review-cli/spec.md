...
## Purpose
Give AI agents a small, safe command-line interface to an in-progress review, so
they act on feedback through validated commands rather than hand-editing the
state JSON. The `code-review` binary stays GUI-first — a bare invocation opens
the GUI — and exposes the CLI only when called with a subcommand. The CLI is
read-only on the repository (its sole write target is the review state file),
resolves the review exactly as the GUI does (no flags), and emits purpose-built
JSON so agents never need to know the state-file schema. The `instructions`
command is the agent's single source of contract.
